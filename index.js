...
async function startelaina() {
    ...
    elaina.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./elaina.js")(elaina, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
//Wel = await getBuffer(`http://hadi-api.herokuapp.com/api/card/welcome3?username=${encodeURIComponent(nama)}&memberCount=${encodeURIComponent(memb)}&gcname=${encodeURIComponent(metadata.subject)}&bg=https://telegra.ph/file/38dfc6649ab96b7dc7d41.jpg&pp=${encodeURIComponent(ppuser)}&gcicon=${encodeURIComponent(ppgroup)}`)
//Welcome = await getBuffer(`https://api.lolhuman.xyz/api/base/welcome?apikey=3f1dc1eb9419577d26f2fafa&img1=${ppuser}&img2=${ppgroup}&background=https://telegra.ph/file/38dfc6649ab96b7dc7d41.jpg&username=${nama}&member=${encodeURIComponent(memb)}&groupname=${encodeURIComponent(metadata.subject)}`)
...
//Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/045471598515af3cbed67.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                   ...
                   if (anu.action == 'add') {
                    elaina.sendMessage(anu.id, { image: Welcom, contextInfo: { mentionedJid: [num] }, caption: `Welcome @${num.split("@")[0]}`})
                } else if (anu.action == 'remove') {
                    elaina.sendMessage(anu.id, { image: Leav, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
